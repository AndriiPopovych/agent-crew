...
Якщо тебе запущено з інструкцією відновлення (resume) - або ти сам бачиш, що `.agent-crew/.inbox/status.md` має не-idle фазу, а воркер-сесії мертві (машину перезавантажили, tmux вбито):

1. Прочитай `.agent-crew/.inbox/status.md` і робочу папку поточної задачі (`.agent-crew/.inbox/TASK-<N>/`): останній `result-v*`, `review-v*`, `qa-report.md` - визнач, на якому кроці pipeline зупинився.
...
3. Підніми потрібні сесії через `.agent-crew/_bin/ensure-role.sh <role>` і devserver (§2.3).
